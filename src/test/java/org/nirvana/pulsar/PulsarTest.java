...
public class PulsarTest {
    ...
    // 王柱林的pulsar
    String serviceUrl = "pulsar://8.136.80.168:6650";
     ...
     */
    @Test
    public void testConsumerSync() throws IOException {
        String topic = "my-topic";
        String consumerName = "first-consumer1";

        consumer = client.newConsumer().topic(topic).subscriptionName(consumerName).subscribe();
        Message<byte[]> message = consumer.receive();
//        consumer.acknowledge(message);
//        log.error("Received message: {}, messageId: {}", message.getMessageId(), new String(message.getData()));
        ...
        executor.execute(() -> {
            ...
            log.error("Received message: {} messageId: {}", new String(message.getData()), message.getMessageId());
